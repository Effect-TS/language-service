...
// Effect.try
export const uc_try = Effect.try({ try: () => JSON.parse("{}"), catch: (e) => new Error(String(e)) })
...
// Effect.filterMap — v4 uses Filter.FilterEffect; simplified example
export const uc_filterMap = Effect.filterMap([1, 2, 3], (n) => Effect.succeed(n > 1 ? Option.some(n * 2) : Option.none()))
...
// Effect.catch — v4 name (renamed from catchAll)
export const uc_catch = Effect.catch(Effect.fail("err") as Effect.Effect<number, string>, (_e) => Effect.succeed(0))
...
// Effect.catchTag
class MyError extends ServiceMap.Service<MyError, { message: string }>()("MyError") {}
...
// Effect.provide
const SomeLayer = { build: (_: any) => Effect.succeed({}) } as any
export const uc_provide = Effect.provide(Effect.succeed(1), SomeLayer)

// Effect.provideService
const SomeService = ServiceMap.Service<{ value: number }>("SomeService")
...
// Effect.logWithLevel
export const uc_logWithLevel = Effect.logWithLevel("Info")("custom level message")
...
// Effect.clockWith
export const uc_clockWith = Effect.clockWith((clock) => clock.currentTimeMillis)
