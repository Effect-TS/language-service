...
export class NanoDefectException extends Data.TaggedError("NanoDefectException")<{ value: unknown }>{}
...
export const contextAdd = <R, I extends NanoTag<any>>(context: NanoContext<R>, tag: I, value: I["~nano.requirements"]): NanoContext<R | I> => ({
    _tag: "Cons",
    key: tag.key,
    value,
    next: context
})
export const contextGet = <R, I extends NanoTag<any>>(context: NanoContext<R | I>, tag: I): Option.Option<I["~nano.requirements"]> => {
    let current = context
...
export class Nano<out A, out E = never, out R = never>{
    declare readonly "~nano.variance": {
        _A: () => A,
        _E: () => E,
        _R: () => R
    }
    ...
    constructor(
        public readonly run: (ctx: NanoContext<unknown>) => Either.Either<A, E | NanoInterruptedException | NanoDefectException>
    ){}
...
export const service = <I extends NanoTag<any>>(tag: I) => new Nano<I["~nano.requirements"], never, I["~nano.requirements"]>((ctx) => contextGet(ctx, tag).pipe(Option.match({
    onNone: () => Either.left(new NanoDefectException({ value: `Cannot find service ${tag.key}`})),
...
export const gen = <Eff extends Gen.YieldWrap<Nano<any, any, any>>, AEff>(
  ...
  AEff,
  [Eff] extends [never] ? never : [Eff] extends [Gen.YieldWrap<Nano<infer _A, infer E, infer _R>>] ? E : never,
  [Eff] extends [never] ? never : [Eff] extends [Gen.YieldWrap<Nano<infer _A, infer _E, infer R>>] ? R : never
>((ctx => {
